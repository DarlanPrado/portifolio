<template>
  ...
  <div class="h-screen">
    ...
    <div :class="colorControl.value == 'light' ? 'grid' : ''" class="h-full w-full z-30 absolute bg-transparent backdrop-blur-sm">
      <div class="w-full h-full flex flex-col bg-gradient-to-b from-transparent to-gray-200 dark:to-gray-800" @mousemove="moveDiv" @mouseenter="showCursorDivEffect = true" @mouseleave="showCursorDivEffect = false">
        <div class="h-full flex justify-center items-center">
          <div class="p-5">
            ...
            <h3 class="text-4xl text-center">Desenvolvedor Fullstack</h3>
            <div class="flex justify-center items-center mt-2">
              <div class="flex gap-5">
                <UTooltip text="Linkedin">
                  <ULink :to="useLinksStore().getLinks['linkedin']" aria-label="Acesse meu perfil do Linkedin" active-class="text-primary" target="_blank" >
                    <UIcon name="i-logos-linkedin-icon" class="w-10 h-10" />
                  </ULink>
                </UTooltip>
                <UTooltip text="Github">
                  <ULink :to="useLinksStore().getLinks['github']" aria-label="Acesse meu perfil do Github" active-class="text-primary" target="_blank" >
                    <UIcon name="i-logos-github-icon" class="w-10 h-10"/>
                  </ULink>
                </UTooltip>
                <UTooltip text="Gmail">
                  <ULink :to="useLinksStore().getLinks['email']" aria-label="Entre em contato via Email" active-class="text-primary" target="_blank" >
...
const colorControl = useColorMode();
